...
def main():
    ...
    splitter = DataSplitter(
        annotation_file_path=os.path.join(ANNOTATIONS_DIR, ORIGINAL_ANNOTATIONS_DIR, ORIGINAL_TRAIN_ANNOTATION_FILE),
        train_split_percentage=train_split_percentage,
        train_output_file=os.path.join(ANNOTATIONS_DIR, NEW_ANNOTATIONS_DIR,
                                       'annotations_file_model_idx_{model_idx}'.format(model_idx=initial_model_idx)),
        test_output_file=os.path.join(ANNOTATIONS_DIR, NEW_ANNOTATIONS_DIR,
                                      'unused_annotations_file_model_idx_{model_idx}'.format(
                                          model_idx=initial_model_idx)))

    splitter.split()
    for curr_model_idx, train_split_percentage in zip(range(initial_model_idx + 1, STUDENT_TEACHER_LOOP - 1),
                                                      TRAIN_SPLIT_PERCENTAGES[1:]):
        annotation_file_path = os.path.join(ANNOTATIONS_DIR, NEW_ANNOTATIONS_DIR,
                                            'unused_annotations_file_model_idx_{prev_mode_idx}'.format(
                                                prev_mode_idx=curr_model_idx - 1))
        train_output_file = os.path.join(ANNOTATIONS_DIR, NEW_ANNOTATIONS_DIR,
                                         'annotations_file_model_idx_{curr_model_idx}'.format(
                                             curr_model_idx=curr_model_idx))
        if curr_model_idx == STUDENT_TEACHER_LOOP - 2:
            test_output_file = os.path.join(ANNOTATIONS_DIR, NEW_ANNOTATIONS_DIR,
                                            'annotations_file_model_idx_{curr_model_idx}'.format(
                                                curr_model_idx=curr_model_idx + 1))
        else:
            test_output_file = os.path.join(ANNOTATIONS_DIR, NEW_ANNOTATIONS_DIR,
                                            'unused_annotations_file_model_idx_{curr_model_idx}'.format(
                                                curr_model_idx=curr_model_idx))
    ...
    for model_idx in range(0, STUDENT_TEACHER_LOOP - 2):
        os.remove(os.path.join(ANNOTATIONS_DIR, NEW_ANNOTATIONS_DIR,
                               'unused_annotations_file_model_idx_{model_idx}'.format(model_idx=model_idx)))
