...
from consts import TRAIN_COMMAND_TEACHER
...
class Teacher(Model):
    def __init__(self, model_type, model_idx, num_train_epochs, train_image_dir, train_annotations, val_image_dir, val_annotations, next_gen_annotations, second_next_gen_annotations, full_data_model=False):
        super().__init__(model_type, model_idx, num_train_epochs, train_image_dir, train_annotations, val_image_dir, val_annotations, next_gen_annotations, second_next_gen_annotations, full_data_model)

    def fit(self):
        train_images_count = self.get_images_count_in_train_annotations_file()
        train_process_return_value = os.system(TRAIN_COMMAND_TEACHER.format(openpifpaf_path=OPENPIFPAF_PATH,
        ...
        if train_process_return_value != 0:
            raise ValueError('Training Teacher failed')
